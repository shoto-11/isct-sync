  ...
  return (
    <div style={{ background: "#F4F6F5", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "24px 16px" }}>
        ...
        {activeTab === "events" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <h2 style={{ fontSize: 16, fontWeight: 700 }}>イベント一覧（{events.length}件）</h2>
            ...
            {events.filter((e) => !eventSearch || e.title?.includes(eventSearch) || e.organizerName?.includes(eventSearch)).map((event) => (
              <div key={event.id} style={adS.listItem}>
                ...
                <div style={{ flex: 1, cursor: "pointer" }} onClick={() => navigate(`/events/${event.id}`)}>
                  <div style={{ fontSize: 14, fontWeight: 700 }}>{event.title}</div>
                  <div style={{ fontSize: 11, color: "#5A7370", display: "flex", alignItems: "center", gap: 6 }}><Calendar size={11} /> {event.date} <User size={11} /> {event.organizerName}</div>
                </div>
                <button style={{ padding: "6px 16px", borderRadius: 999, border: "1.5px solid #E53935", background: "white", color: "#E53935", fontSize: 12, fontWeight: 700, cursor: "pointer" }}
                  onClick={() => handleDeleteEvent(event.id, event.title)}>削除</button>
              </div>
            ))}
          </div>
        )}
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button style={{ flex: 1, padding: 12, background: THEME, color: "white", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer" }}
                      onClick={async () => {
                        ...
                        setSelectedUser(null); setEditingUser(null); alert("保存しました！");
                      }}>保存する</button>
                  </div>
                </div>
              </div>
