...
export default function Login() {
  ...
  const handleSendLink = async () => {
    ...
    if (!allowed) { setError("このメールアドレスは使用できません"); return; }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const allowed = await checkAllowed(userEmail);
      if (!allowed) {
        await auth.signOut();
        setError(`このアカウント（${userEmail}）はご利用いただけません`);
        setLoading(false);
        return;
      }
      ...
      <div style={s.card}>
        ...
        {step === "email" ? (
          <>
            ...
            <div style={s.note}>@m.isct.ac.jp の学籍アカウントのみ利用できます</div>
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 80, height: 80, background: "#F9EAED", borderRadius: "50%" }}>
              <Mail size={40} color={THEME} />
            </div>
            <h2 style={s.sentTitle}>メールを確認してください</h2>
            <p style={s.sentText}>
              <strong>{email}</strong> にログイン用リンクを送信しました。メール内のリンクをクリックしてログインしてください。
            </p>
            <div style={s.note}>メールが見つからない場合は迷惑メールフォルダをご確認ください</div>
            <button style={s.outlineBtn} onClick={() => { setStep("email"); setError(""); }}>
              別のメールアドレスで試す
            </button>
          </>
        )}
...
const s = {
  ...
  outlineBtn: { background: "none", border: `1px solid ${THEME}`, color: THEME, borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer" },
